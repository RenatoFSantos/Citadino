...
export class UsuarioSqlService {
  ...
  private dataBase: SQLiteObject;
    ...
    constructor(private plt: Platform, private sqlSrv: SqlLiteService) { }
    ...
    InitDatabase() {
      ...
      if (this.plt.is('ios')) {
        this.sqlSrv.createDataBaseIos().then((db: SQLiteObject) => {
          ...
          .catch((err) => {
            throw new Error('Não foi possível abri o banco de dados');
          });
      }
      else {
        this.sqlSrv.createDataBaseAndroid().then((db: SQLiteObject) => {
      ...
      // this.dataBase = sqlSrv.getDataBase();
